...
export const ProfilePage = () => {
    ...
    const UploadPost = () => {
        ...
        .then(response => {
            PostService.getApiPosts(renderedUser?.id as number)
            .then(response => {
    ...
    React.useEffect(() => {
        if(params.id){
            UserService.getApiUsers(parseInt(params.id))
            .then(response => {
                console.log(response)
                dispatch(setRenderedUser({...response}))
                PostService.getApiPosts(response.id as number)
                .then(response => {
                    console.log(response)
                    dispatch(setPosts(response))
                })
            ...
            if(currentUser){
                ...
                PostService.getApiPosts(currentUser.id as number)
                .then(response => {
                    console.log(response)
                    dispatch(setPosts(response))
                })
    ...
    <div className={styles.layout}>
        <div className={styles.profileWindow}>
            <div className={styles.leftSide}>
                <div className={styles.avatar}>
                    <img src={renderedUser?.avatarUrl !== '' ? renderedUser?.avatarUrl ?? EmptyAvatar : EmptyAvatar} alt="avatar" />
                </div>   
                {renderedUser?.id !== currentUser?.id && currentUser?.friends?.every(f => f.id !== renderedUser?.id) &&
                    <Button text="Friend" onClick={AddFriend}/>
                }
                {renderedUser?.id !== currentUser?.id && currentUser?.friends?.some(f => f.id === renderedUser?.id) &&
                    <Button text="Unfriend" onClick={RemoveFriend}/>
                }
            </div>
            <div className={styles.info}>
                <div className={styles.name}>
                    {renderedUser?.name ?? 'Unknown'}
                </div>
                ...
                {renderedUser?.id === currentUser?.id &&
                    <>
                        <input type="file" name="myImage" onChange={onAvatarChange} accept="image/*" />
        ...
        <div className={styles.addPostWindow}>
            ...
            <div className={styles.addPostForm}>
                <textarea style={{resize: "none"}} onChange={event => dispatch(setPostText(event.target.value))}/>
                Image:
        ...
        {modalOpened &&
            <Modal name='Edit info' onModalClose={() => {dispatch(setEditModalStatus(false))}}>
                <Formik
                initialValues={{ name: currentUser?.name, age: currentUser?.age, city: currentUser?.city, education: currentUser?.education }}
                validate={values => {
                    const errors: Partial<typeof values> = {}
                    return errors;
                }}
                onSubmit={(values, { setSubmitting }) => {
                    UserService.postApiUsersUpdate({name: values.name, age: values.age, city: values.city, education: values.education})
                    .then(response => {
                        console.log(response)
                ...
                {({
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <input
                            ...
                            value={values.age}
                        />
                        <div className={styles.errors}>{errors.age && touched.age && errors.age}</div>
                        <input
                            placeholder='City'
                            type="text"
                            name="city"
                        ...
                        />
                        <div className={styles.errors}>{errors.city && touched.city && errors.city}</div>
                        <input
                        ...
                        />
                        <div className={styles.errors}>{errors.education && touched.education && errors.education}</div>
